...
=end
    ...
    line = "<div class='event col-xs-12 firstTd clearfix' data-name-search='#{ event.name }' data-department-search='#{ event.department }' data-type-search='#{event.races.map(&:race_type).reject(&:blank?).join(' ')}' data-month-search='#{get_month(event.editions.last.date)}'>"
      ...
      line += "<ul class='list-inline'>"
        line += "<li><div class='logo-edition'><img src='https://s3-eu-west-1.amazonaws.com/widget-dev.kapp10.com/icoTrailModalite.png'>#{ event.races.map(&:race_type).reject(&:blank?).join(', ') }</div></li>"
        line += "<li><div class='logo-edition'><img src='https://s3-eu-west-1.amazonaws.com/widget-dev.kapp10.com/icoLocality.png'>#{ event.department }</div></li>"
        line += "<li><div class='logo-edition'><img src='https://s3-eu-west-1.amazonaws.com/widget-dev.kapp10.com/icoDistance.png'>#{ event.races.map(&:name).uniq.join(', ') }</div></li>"
      line += "</ul>"
      line += "</div>"
      ...
      line += "<div class='theDate'>"
        ...
        line += "<ul class='list-inline'>"
          line += "<li>"
            ...
            line += "<div class='results-count'><div>#{ "#{event.editions.last.results.count} <span>participants</span>" if event.editions.last.results.count > 0 }</div></div>"
          line += "</li>"
