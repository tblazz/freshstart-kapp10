...
function participant_filter() {
  var input, tr, i, n, reset_filter, filters, linesToDisplay = "", lines;
  filters = get_filters();
  input = document.getElementById("kapp10-filter");
...
$(document).ready(function(){
  const categoriesSortedDiv = document.getElementById("categories-sorted")
  categoriesSorted          = [JSON.parse(categoriesSortedDiv.dataset.json)]
  
  const editionsLinesDiv    = document.getElementById("editions-lines")
  edition_lines             = [JSON.parse(editionsLinesDiv.dataset.json)][0]
  
  const longestNameDiv      = document.getElementById("longest-name")
  longestName               = [JSON.parse(longestNameDiv.dataset.json)]
